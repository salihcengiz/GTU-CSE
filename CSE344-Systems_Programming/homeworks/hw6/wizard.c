/*
 * wizard.c - Wizard Client
 * CSE344 HW6 - Connects to Hogwarts server as a WIZARD
 */
...
int main(int argc, char *argv[]) {
    ...
    snprintf(enroll_cmd, sizeof(enroll_cmd), "ENROLL WIZARD %s", username);
    ...
    while (running) {
        if (quit_flag) {
            if (!apparate_sent) {
                ...
                printf("[WIZARD %s] SENT APPARATE\n", username);
                fflush(stdout);
                apparate_sent = 1;
            }

            /* try to read OK APPARATE */
            char tmp[MAX_LINE];
            ...
            printf("[WIZARD %s] DISCONNECTED reason=APPARATE\n", username);
            fflush(stdout);
            break;
        }
                ...
                if (tmp[i] == '\n') {
                    ...
                    if (srv_buf_len > 0) {
                        ...
                        if (strcmp(srv_buf, "SERVER SHUTDOWN") == 0) {
                            printf("[WIZARD %s] DISCONNECTED reason=shutdown\n", username);
                            fflush(stdout);
                            running = 0;
                        } else if (strcmp(srv_buf, "TIMEOUT DISCONNECT") == 0) {
                            printf("[WIZARD %s] DISCONNECTED reason=timeout\n", username);
                            fflush(stdout);
                            running = 0;
                        } else if (strcmp(srv_buf, "ERR HOGWARTS_FULL") == 0) {
                            printf("[WIZARD %s] DISCONNECTED reason=full\n", username);
                            fflush(stdout);
                            running = 0;
                        }
                    }
