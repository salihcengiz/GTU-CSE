...
shape* read(char *filename) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        sscanf(line, "%f,%f,%f,%f", &new_shape->x[0], &new_shape->x[1], &new_shape->x[2], &new_shape->x[3]);
        fgets(line, sizeof(line), fp);
        
        sscanf(line, "%f,%f,%f,%f", &new_shape->y[0], &new_shape->y[1], &new_shape->y[2], &new_shape->y[3]);
...
float calculate_triangle_area(float x[], float y[]) {
    ...
    return 0.5 * fabs(x[0]*(y[1] - y[2]) + x[1]*(y[2] - y[0]) + x[2]*(y[0] - y[1]));
}
