...
shape* read(char *filename) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        strncpy(first,line,1);

        if (first == "t"){

            sscanf(line, "%3[^|]%f%f%f", garbage, &new_shape->x[0], &new_shape->x[1], &new_shape->x[2]);
        
            fgets(line, sizeof(line), fp);
        
            sscanf(line, "%s,%f,%f,%f", garbage, &new_shape->y[0], &new_shape->y[1], &new_shape->y[2]);
